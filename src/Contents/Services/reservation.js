...
reservation={
	...
	{
		console.log('select id_salle, nomSalle, debutRessource, finRessource from ressourcesalles r left join module m on r.id_module=m.id_module left join salle on salle.id_salle=r.id_salle left join session s on m.id_session=s.id_session left join evenement e on s.id_evenement=e.id_evenement left join site on r.id_site= site.id_site left join agents on agents.Id=s.chefProjet left join typologie t on t.id_typologie=e.id_typologie where e.status<>"D" and r.status<>"D" and m.status<>"D" and ((debutRessource between "' + o.DebutRessource + '" and "' + o.FinRessource + '") or (finRessource between "' + o.DebutRessource + '" and "' + o.FinRessource + '") or ("' + o.DebutRessource + '" between debutRessource and finRessource) or ("' + o.FinRessource + '" between debutRessource and finRessource))');
        reservation.using('db').model('reservation_salles', 'select id_salle, nomSalle, debutRessource, finRessource from ressourcesalles r left join module m on r.id_module=m.id_module left join salle on salle.id_salle=r.id_salle left join session s on m.id_session=s.id_session left join evenement e on s.id_evenement=e.id_evenement left join site on r.id_site= site.id_site left join agents on agents.Id=s.chefProjet left join typologie t on t.id_typologie=e.id_typologie where e.status<>"D" and r.status<>"D" and m.status<>"D" and ((debutRessource between "' + o.DebutRessource + '" and "' + o.FinRessource + '") or (finRessource between "' + o.DebutRessource + '" and "' + o.FinRessource + '") or ("' + o.DebutRessource + '" between debutRessource and finRessource) or ("' + o.FinRessource + '" between debutRessource and finRessource))', cb);
	},
